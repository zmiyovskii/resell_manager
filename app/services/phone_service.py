...
class PhoneService:
    ...
    def delete_phone(self, db: Session, phone_id: int) -> bool:
        phone = phone_repository.get_by_id(db, phone_id)
        if phone is None:
            return False

        expense_repository.delete_by_phone_id(db, phone_id)
        inventory_repository.delete_movements_by_phone_id(db, phone_id)
        ...
        phone_repository.delete(db, phone)
        return True
